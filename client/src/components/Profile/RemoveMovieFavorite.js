...
const RemoveMovieFavorite = ({ handleVisible, isVisible, movieIdSelected }) => {
    const dispatch = useDispatch()
    const currentUser = useSelector((state) => state.auth.currentUser)

    const handleRemoveMovie = () => {
        dispatch(FavoriteStart({ token: currentUser.accessToken, movieId: movieIdSelected, userId: currentUser._id }))
        handleVisible()
    }
...
export default RemoveMovieFavorite